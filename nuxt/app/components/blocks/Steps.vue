...
import type { BlockStep, BlockStepItem, BlockButtonGroup, BackgroundType } from '~/types/';
...
<template>
  <BlockContainer :background="background">
    <TypographyTitle v-if="data.title" :data-directus="setAttr({ collection: 'block_steps', item: data.id, fields: 'title', mode: 'popover' })">{{ data.title }}</TypographyTitle>
    <TypographyHeadline v-if="data.headline" :content="data.headline" size="lg" :data-directus="setAttr({ collection: 'block_steps', item: data.id, fields: 'headline', mode: 'popover' })" />
    <div class="mt-8" :data-directus="setAttr({ collection: 'block_steps', item: data.id, fields: 'steps', mode: 'modal' })">
      <template v-for="(step, stepIdx) in steps" :key="stepIdx">
        ...
        >
          <div v-if="step.image" class="shrink-0 bg-default rounded-panel dark:brightness-90">
            <NuxtImg
              v-if="step.image"
