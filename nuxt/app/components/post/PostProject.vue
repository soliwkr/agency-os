...
<template>
  <div class="py-12">
    <header class="relative h-[400px] overflow-hidden flex justify-center items-center" :data-directus="setAttr({ collection: 'posts', item: page?.id, fields: 'image', mode: 'modal' })">
      ...
      <div class="relative max-w-3xl p-8 mx-auto overflow-hidden bg-inverted/50 rounded-card">
        <TypographyHeadline :content="page?.title" class="text-inverted" size="xl" :data-directus="setAttr({ collection: 'posts', item: page?.id, fields: 'title', mode: 'popover' })" />
        <TypographyProse :content="page?.summary" class="mt-4 text-inverted" :data-directus="setAttr({ collection: 'posts', item: page?.id, fields: 'summary', mode: 'popover' })" />
      </div>
    </header>
    ...
    <BlockContainer class="md:flex gap-8">
      ...
      <main class="p-4">
        ...
        <div :data-directus="setAttr({ collection: 'posts', item: page?.id, fields: 'gallery', mode: 'modal' })">
          <VGallery v-if="galleryItems && galleryItems.length > 0" :items="galleryItems" />
        </div>
      </main>
      <!-- Project Metadata -->
      <aside class="md:w-[300px] shrink-0">
        <div class="p-4 space-y-8 border-2 border-default rounded-card" :data-directus="setAttr({ collection: 'posts', item: page?.id, fields: ['client', 'built_with', 'cost'], mode: 'modal' })">
          <div>
